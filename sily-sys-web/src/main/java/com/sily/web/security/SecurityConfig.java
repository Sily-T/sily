...
import com.sily.service.UserDetailsServiceImpl;
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.exceptionHandling().accessDeniedPage("/error/403") //访问被拒绝页面的URL
                ...
                .authorizeRequests()//该方法所返回的对象的方法来配置请求级别的安全细节
                    .antMatchers("/html/login.html").permitAll()//对于登录路径不进行拦截
                    .antMatchers("/html/index.html").hasAuthority("管理员")
...
//                    .loginPage("/templates/photoshoot_default.html")//登录页面的访问路径
                    .loginProcessingUrl("/user/login*")//登录页面下表单提交的路径
    ...
    //                .failureHandler(new AjaxLoginFailureHandler()).and()
                    .failureUrl("/html/login.html")//登录失败后跳转的路径
                    .defaultSuccessUrl("/html/index.html")//登录成功后默认跳转的路径
